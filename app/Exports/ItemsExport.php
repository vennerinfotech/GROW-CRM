...
class ItemsExport implements FromCollection, WithHeadings, WithMapping {
            ...
            foreach (request('standard_field') as $key => $value) {
                if ($value == 'on') {
                    ...
                    case 'item_description':
                        //strip HTML tags and replace <p> and <br> with newlines
                    ...
                    case 'item_notes_estimatation':
                        //strip HTML tags and replace <p> and <br> with newlines
                        $text = $items->item_notes_estimatation ?? '';
                        $text = str_replace(['<p>', '</p>', '<br>', '<br/>', '<br />'], ["\n", "\n", "\n", "\n", "\n"], $text);
                        $map[] = strip_tags($text);
                        break;
                    case 'count_sold':
    ...
    public function headings(): array
    {
        ...
        $standard_lang = [
            ...
            'item_description' => __('lang.name'),
            'item_notes' => __('lang.description'),
        ...
        if (is_array(request('custom_field'))) {
            foreach (request('custom_field') as $key => $value) {
                if ($value == 'on') {
                    //get custom field name from database
                    $fieldId = str_replace('item_custom_field_', '', $key);
                    $customField = \App\Models\ProductCustomField::where('items_custom_id', $fieldId)->first();
                    $heading[] = $customField ? $customField->items_custom_field_name : $key;
                }
            }
        }
