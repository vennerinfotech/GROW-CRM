...
class Items extends Controller {
    ...
    public function __construct(ItemRepository $itemrepo, UserRepository $userrepo, UnitRepository $unitrepo, ProductTaskRepository $producttaskrepo) {
        ...
        //Filtering
        $this->middleware('itemsMiddlewareFiltering')->only([
            'index',
        ]);
    ...
    public function create(CategoryRepository $categoryrepo) {
        ...
        //tax rates
        $taxrates = \App\Models\TaxRate::Where('taxrate_status', 'enabled')->orderBy('taxrate_name', 'asc')->get();

        //get enabled product custom fields
        $custom_fields = ProductCustomField::Where('items_custom_field_status', 'enabled')
            ->orderBy('items_custom_id', 'asc')
            ->get();
    ...
    public function store(ItemStoreUpdate $request, UnitRepository $unitrepo) {
        ...
        //handle unit_id (could be existing ID or new unit name)
        $unit_input = request('item_unit');

        //check if it's numeric (existing unit ID) or text (new unit name)
        if (is_numeric($unit_input)) {
            //it's an existing unit ID - use as is
            request()->merge(['item_unit' => (int)$unit_input]);
        } else {
            //it's text - create new unit and get its ID
            $unit_id = $unitrepo->createIfNotExists($unit_input);
            if ($unit_id) {
                request()->merge(['item_unit' => $unit_id]);
            } else {
        ...
        event(new \App\Events\Items\ItemStoring(request()->all(), $request->validated()));

        //merge custom fields data
        $custom_fields_data = [];
        for ($i = 1; $i <= 10; $i++) {
            if (request()->has('item_custom_field_' . $i)) {
                $custom_fields_data['item_custom_field_' . $i] = request('item_custom_field_' . $i);
            }
        }
        if (!empty($custom_fields_data)) {
            request()->merge($custom_fields_data);
        }
        ...
        event(new \App\Events\Items\ItemStored($item_id, $item, request()->all()));
    ...
    public function update(ItemStoreUpdate $request, UnitRepository $unitrepo, $id) {
        ...
        //fire post-storage event for module data saving
        event(new \App\Events\Items\ItemStored($id, $item, request()->all()));
    ...
    public function destroy() {
        ...
        foreach (request('ids') as $id => $value) {
            ...
            if ($value == 'on') {
                ...
                // Broadcast ItemDeleting event before deletion
                event(new \App\Events\Items\ItemDeleting($item));
        ...
        event(new \App\Events\Items\ItemDeleted(request(), $allrows, $payload));
    ...
    public function bulkDelete() {
        ...
        foreach (request('item') as $item_id => $value) {
            if ($value == 'on') {
                ...
                if ($items = $this->itemrepo->search($item_id)) {
                    ...
                    // Broadcast ItemDeleting event before deletion
                    event(new \App\Events\Items\ItemDeleting($items->first()));
        ...
        //fire post-action event
        event(new \App\Events\Items\ItemBulkDeleted(request(), $deleted_ids, $jsondata));
    ...
    public function changeCategoryUpdate(CategoryRepository $categoryrepo) {
        ...
        //fire post-action event
        event(new \App\Events\Items\ItemCategoryChanged(request(), $item_ids, $payload));
    ...
    public function storeTask(StoreUpdateTask $request) {
        ...
        event(new \App\Events\Items\ItemTaskStoring(request()));
        ...
        //fire post-action event
        event(new \App\Events\Items\ItemTaskStored(request(), $task->product_task_id, $payload));
    ...
    public function updateTask(StoreUpdateTask $request, $id) {
        ...
        //fire pre-action event
        event(new \App\Events\Items\ItemTaskUpdating(request(), $id));
        ...
        //fire post-action event
        event(new \App\Events\Items\ItemTaskUpdated(request(), $id, $payload));
    ...
    public function destroyTask($id) {
        ...
        //fire pre-action event
        event(new \App\Events\Items\ItemTaskDeleting(request(), $id));
        ...
        //fire post-action event
        event(new \App\Events\Items\ItemTaskDeleted(request(), $id, $payload));
    ...
    public function togglePinning(PinnedRepository $pinrepo, $id) {
        ...
        //fire post-action event
        event(new \App\Events\Items\ItemPinToggled(request(), $id, $payload));
