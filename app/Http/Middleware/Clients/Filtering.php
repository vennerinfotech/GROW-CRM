...
/** --------------------------------------------------------------------------------
 * This middleware class handles filter processing for clients
 *
 * @package    Grow CRM
 * @author     NextLoop
 *----------------------------------------------------------------------------------*/
...
class Filtering {
    ...
    public function handle($request, Closure $next) {
        ...
        //filter handling
        if (request()->routeIs('clients.search')) {
    ...
    private function saveFilter() {
        ...
        if (!$filter = \App\Models\Filter::where('filter_userid', auth()->id())
            ->where('filter_type', 'clients')->first()) {
            $filter = new \App\Models\Filter();
            $filter->filter_userid = auth()->id();
            $filter->filter_type = 'clients';
