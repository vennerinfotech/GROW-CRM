...
class InvoiceStoreUpdate extends FormRequest {
    ...
    public function rules() {
        ...
        /**-------------------------------------------------------
         * [create] due date validation (conditional on method)
         * ------------------------------------------------------*/
        if ($this->getMethod() == 'POST') {
            $rules += [
                'invoice_due_date_method' => [
                    'required',
                    Rule::in(['set_due_date_automatically', 'set_due_date_manually']),
                ],
            ];

            //if manual mode, require and validate due date
            if (request('invoice_due_date_method') == 'set_due_date_manually') {
                ...
                //if automatic mode, due date is nullable (will be calculated)
                $rules += [
                    'bill_due_date' => [
                        'nullable',
                    ],
