...
$(document).ready(function () {
  ...
  $(document).on("click", ".js-add-cover-image", function () {
    ...
    $("#card-cover-image-container").css(
      "background-image",
      'url("' + image_url + '")'
    );

    //set the cover on the kanban also
    $("#kanban-card-cover-image-" + kanban_card_id).css(
      "background-image",
      'url("' + image_url + '")'
    );
  ...
  $(document).on("click", "#card-modal-close", function () {
    ...
    // Check if we are in a lead modal view (leads/v/{id}...)
    if (currentUrl.includes("/leads/v/")) {
      // Push the base leads URL to history
      // Use standard pushState to avoid refresh
      var baseUrl = "leads"; // Relative or absolute path handling might be needed depending on base
      // A safer approach is to strip the /v/... part
      // Example: http://127.0.0.1:8000/leads/v/19/new-lead -> http://127.0.0.1:8000/leads
      var newUrl = currentUrl.split("/v/")[0];
