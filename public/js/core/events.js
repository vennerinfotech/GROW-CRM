...
$(document).ready(function () {
  ...
  function preselectFilterSelect2Ajax() {
    $(
      ".js-select2-basic-search, .js-select2-basic-search-modal, .js-select2-dynamic-project"
    ).each(function () {
      var $select = $(this);
  ...
  $(document).on(
    ...
    function (e) {
      ...
      $("#manual_timer_created_" + rowId)
        .prop("disabled", true)
        .val("");
      $("#timer_created_edit_" + rowId)
        .prop("disabled", true)
        .val("");
      $("#manual_time_hours_" + rowId)
        .prop("disabled", true)
        .val("");
      $("#manual_time_minutes_" + rowId)
        .prop("disabled", true)
        .val("");
    }
  );
  ...
  $(document).on("click", "#add_timesheet_row", function (e) {
    ...
    $default_row
      .find(".js-select2-basic-search-modal, .projects_my_tasks_toggle")
      .each(function () {
        var $select = $(this);
        var select_id = $select.attr("id");
        ...
        if (saved_values[select_id] && saved_values[select_id].value) {
          ...
          var option = new Option(
            saved_values[select_id].text,
            saved_values[select_id].value,
            true,
            true
          );
    ...
    $new_row.find("input, select").each(function () {
      ...
      if (name) {
        ...
        var new_name = name
          .replace("[default]", "[" + unique_id + "]")
          .replace("_default", "_" + unique_id);
    ...
    $new_row
      .find(".projects_my_tasks_toggle")
      .attr("data-task-dropdown", "my_assigned_tasks_" + unique_id);
    ...
    if (user_id) {
      $new_row
        .find(".projects_my_tasks_toggle")
        .attr("data-ajax--url", "/feed/users-projects?user_id=" + user_id);
    }

    // Clear values
    $new_row
      .find(
        'input[type="text"], input[type="number"], input[type="hidden"]:not([name*="timesheet_user"])'
      )
      .val("");
    ...
    var lang_remove =
      $("#timesheet_rows_container").data("lang-remove") || "Remove";

    // Add delete button
    var delete_btn =
      '<div class="form-group row m-t-15"><div class="col-12 text-right">' +
      '<button type="button" class="btn btn-sm btn-danger remove-timesheet-row" data-row-id="' +
      unique_id +
      '">' +
      '<i class="ti-close"></i> ' +
      lang_remove +
   ...
   * -------------------------------------------------------------------------------------------------*/
  $(document)
    .off("click", ".js-lead-checklist-add-product")
    .on("click", ".js-lead-checklist-add-product", function (e) {
      e.preventDefault();
  ...
  $(document).on(
    "keyup change paste",
    ".items-col-tableconfig_column_5 input.form-control-number",
    function () {
      var row = $(this).closest("tr");
      var rate = parseFloat(
        row
          .find(".items_col_tableconfig_column_2")
          .text()
          .replace(/[^0-9.-]+/g, "")
      ...
      //Format currency (basic formatting, ideally should use system format)
      var formattedTotal = total.toFixed(2);

      //Update the display
      row.find(".js-item-calculated-total").text(formattedTotal);
      row.find(".js-item-total-container").removeClass("hidden");
    }
  );
  ...
  $(document).on("click", ".js-lead-confirm-product-selection", function (e) {
    ...
    if ($selected.length > 0) {
      ...
      $selected.each(function (index) {
        ...
        // Appending hidden inputs for backend processing
        // We append them after #lead_product_id to ensure they are within the form
        var hiddenInputs = 
            '<input type="hidden" name="assigned_products['+index+'][item_id]" value="'+id+'" class="js-hidden-product-input">' +
            '<input type="hidden" name="assigned_products['+index+'][quantity]" value="'+quantity+'" class="js-hidden-product-input">' +
            '<input type="hidden" name="assigned_products['+index+'][name]" value="'+name+'" class="js-hidden-product-input">';
        
        if($("#lead_product_id").length > 0) {
             $("#lead_product_id").after(hiddenInputs);
        } else {
             // Fallback if the specific ID isn't found, though it should be check form
              $btn.closest('.modal').find('form').append(hiddenInputs);
        }


        // Capture the first ID for the hidden field (backend compatibility)
        if (first_id === null) {
          first_id = id;
        }
      });
   ...
   * -------------------------------------------------- */
  $(document)
    .find(".js-elements-popover-button")
    .each(function () {
      $(this).popover({
        html: true,
  ...
  $(document).on("click", "#billing-tax-popover-button", function (e) {
    ...
    if ($(this).hasClass("disabled")) {
      ...
      NXINVOICE.log(
        "[invoicing] initialiseTaxPopover() - tax button is disabled"
      );
    } else {
  ...
  $(document).on(
    "click",
    "#itemsModalSelectButton, #categoryItemsModalSelectButton",
    function (e) {
  ...
  $(document).on(
    "click",
    ".js-do-not-close-on-click > .select2-search__field",
    function (e) {
      e.stopPropagation();
    }
  );
  ...
  $(document).on("click", ".edit-add-modal-button", function () {
    ...
    var button_loading_annimation = $(this).attr(
      "data-button-loading-annimation"
    );
    ...
    //set modal size
    $("#commonModalContainer").removeClass(
      "modal-lg modal-sm modal-xl modal-xs"
    );
  ...
  $(document).on("click", ".create-modal-button", function () {
    ...
    //set create url
    $("#create-new-client-button").attr(
      "data-url",
      $(this).attr("data-new-client-url")
    );
  ...
  $(document).on(
    "click",
    ".card-task-assigned, .card-lead-assigned",
    function (e) {
  ...
  $(document).on(
    "change",
    ".custom-fields-standard-form-required-checkbox",
    function () {
      nxAjaxUxRequest($(this));
    }
  );
  ...
  $(document).on(
    "select2:unselecting",
    ".projects_and_milestones_toggle",
    function (e) {
  ...
  $(document).on(
    "select2:unselecting",
    ".projects_assigned_client_toggle",
    function (e) {
  ...
  $(document).on(
    "change",
    "#settings2_estimates_automation_create_project, #estimate_automation_create_project",
    function () {
      if ($(this).is(":checked")) {
  ...
  $(document).on(
    "change",
    "#settings2_estimates_automation_create_invoice, #estimate_automation_create_invoice",
    function () {
      if ($(this).is(":checked")) {
  ...
  $(document).on(
    "select2:select",
    "#settings2_estimates_automation_default_status, #automation_default_status, #estimate_automation_status",
    function (e) {
      var selection = e.params.data.id;
      //toggle permissions
  ...
  $(document).on(
    "select2:select",
    "#settings2_projects_automation_default_status, #automation_default_status, #project_automation_status",
    function (e) {
      var selection = e.params.data.id;
      //toggle permissions
      if (selection == "enabled") {
  ...
  $(document).on(
    "change",
    "#settings2_projects_automation_create_invoices, #project_automation_create_invoices",
    function () {
      if ($(this).is(":checked")) {
  ...
  $(document).on(
    "change",
    "#settings2_projects_automation_invoice_unbilled_hours, #project_automation_invoice_unbilled_hours",
    function () {
      if ($(this).is(":checked")) {
  ...
  $(document).ready(function () {
    $(document).on("click", "#js-products-automation-tasks", function () {
      ...
      $("#create-product-task-button").attr(
        "data-url",
        $(this).attr("data-create-task-url")
      );
      $("#create-product-task-button").attr(
        "data-action-url",
        $(this).attr("data-create-task-action-url")
      );
      //loadajax
      nxAjaxUxRequest($(this));
    });
  ...
  $(document).ready(function () {
    ...
    $(document).on(
      "click",
      "#bill-file-attachments-upload-button",
      function () {
    ...
    $(document).on(
      "click",
      "#bill-file-attachments-close-button",
      function (event) {
        event.stopPropagation();
  ...
  $(document).on("click", "#ticket_replay_button_inline", function (e) {
    ...
    if (reply_type == "note") {
      ...
      var reply_button_text = $("#ticket_reply_button_submit").attr(
        "data-lang-save-note"
      );
  ...
  $(document).on(
    "mouseout",
    "#admin-dhasboard-income-vs-expenses",
    function () {
  ...
  $(document).on("click", ".publishing_option", function (event) {
    ...
    if ($(this).attr("id") == "publishing_option_later") {
      ...
      $("#publishing_option_button").html(
        $("#publishing_option_button").attr("data-lang-schedule")
      );
    } else if ($(this).attr("id") == "publishing_option_now") {
      $(".publishing_option_date").prop("disabled", true);
      $("#publishing_option_button").html(
        $("#publishing_option_button").attr("data-lang-publish")
      );
    }
  });
  ...
  $(document).on(
    "select2:select",
    "#settings2_proposals_automation_default_status, #automation_default_status, #proposal_automation_status",
    function (e) {
      var selection = e.params.data.id;
      //toggle permissions
      if (selection == "enabled") {
  ...
  $(document).on(
    "select2:select",
    "#settings2_contracts_automation_default_status, #automation_default_status, #contract_automation_status",
    function (e) {
      var selection = e.params.data.id;
      //toggle permissions
      if (selection == "enabled") {
  ...
  $(document).on(
    "change",
    "#settings2_contracts_automation_create_project, #contract_automation_create_project",
    function () {
      if ($(this).is(":checked")) {
        $("#settings_automation_create_project_options").show();
  ...
  $(document).on(
    "change",
    "#settings2_contracts_automation_create_invoice, #contract_automation_create_invoice",
    function () {
      if ($(this).is(":checked")) {
        $("#settings_automation_create_invoice_options").show();
  ...
  $(document).on(
    "change",
    "#settings2_proposals_automation_create_project, #proposal_automation_create_project",
    function () {
      if ($(this).is(":checked")) {
        $("#settings_automation_create_project_options").show();
   ...
   * -------------------------------------------------------------------------------------------------*/
  $(document)
    .off("click", ".on-close-reload-parent")
    .on("click", ".on-close-reload-parent", function (e) {
      //add overlay to page
  ...
  $(document).on("select2:select", "#timesheet_user", function (e) {
    ...
    $("#my_assigned_projects").data("select2").dataAdapter.ajaxOptions.url =
      newUrl;
  ...
  $(document).on(
    "click",
    ".checklist-comments-wrapper-toggle-button",
    function (e) {
      e.preventDefault();

      //toggle text aread
      var text_area_wrapper_id = $(this).attr(
        "data-checklist-comments-textarea-wrapper"
      );
  ...
  $(document).on(
    "click",
    "#checklist-add-new-button, #new-checklist-text-form-close-button",
    function (e) {
  ...
  $(document).on("select2:select", "#bill_clientid", function (e) {
    ...
    $("#invoice_due_date_method")
      .val("set_due_date_automatically")
      ...
      $.ajax({
        ...
        success: function (response) {
          if (response.due_days !== undefined) {
            ...
            $("#bill_due_date_display").attr(
              "data-invoice-due-days",
              response.due_days
            );
        ...
        error: function () {
          ...
          var system_default =
            $("#bill_due_date_display").data("system-default") || 7;
          $("#bill_due_date_display").attr(
            "data-invoice-due-days",
            system_default
          );
  ...
  $(document).on(
    "change",
    "#bill_date_add_edit, .pickadate[name='bill_date_add_edit']",
    function () {
      //Only recalculate if in automatic mode
      if (
        $("#invoice_due_date_method").val() === "set_due_date_automatically"
      ) {
        calculateInvoiceDueDate();
      }
    }
  ...
  window.calculateInvoiceDueDate = function () {
    ...
    var due_days =
      parseInt($("#bill_due_date_display").attr("data-invoice-due-days")) ||
      parseInt($("#bill_due_date_display").data("system-default")) ||
      7;
  ...
  function updateDueDateDisplay(invoice_date, due_days) {
    ...
    var display_text =
      NXLANG.invoice_date_plus + " " + due_days + " " + day_label;
    $("#bill_due_date_display").val(display_text);

    //Update hidden field
    $("#bill_due_date_auto_container")
      .find("#bill_due_date")
      .val(due_date_mysql);
  }
...
function NXInvoiceCalculateInitialDueDate() {
  ...
  var dueDaysText = $("#js-trigger-invoices-modal-add-edit").data(
    "due-days-text"
  );
