    ...
    <div id="list-page-actions">
        ...
        <span class="dropdown">
            ...
            <div class="dropdown-menu" aria-labelledby="listTableAction">
                ...
                <!--attach project-->
                <a class="dropdown-item confirm-action-danger {{ runtimeVisibility('dettach-proposal', $document->bill_projectid)}}"
                    href="javascript:void(0)" data-confirm-title="{{ cleanLang(__('lang.detach_from_project')) }}"
                    id="bill-actions-dettach-project" data-confirm-text="{{ cleanLang(__('lang.are_you_sure')) }}"
                    data-url="{{ urlResource('/proposals/'.$document->doc_id.'/detach-project') }}">
                    {{ cleanLang(__('lang.detach_from_project')) }}</a>

                <!--deattach project-->
                <a class="dropdown-item actions-modal-button js-ajax-ux-request reset-target-modal-form {{ runtimeVisibility('attach-proposal', $document->bill_projectid)}}"
                    href="javascript:void(0)" data-toggle="modal" data-target="#actionsModal"
                    id="bill-actions-attach-project" data-modal-title="{{ cleanLang(__('lang.attach_to_project')) }}"
                    data-url="{{ urlResource('/proposals/'.$document->doc_id.'/attach-project') }}"
                    data-action-url="{{ urlResource('/proposals/'.$document->doc_id.'/attach-project') }}"
                    data-loading-target="actionsModalBody" data-action-method="POST">
