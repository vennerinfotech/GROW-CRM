        ...
        <div class="col-6 text-right">
            <div class="p-r-0">
                <ul>
                    ...
                    <!--add signature - authenticated client users-->
                    @if(config('visibility.doc_client_add_signature') && auth()->check() && !auth()->user()->is_team)
                    <li>
                        <button type="button"
                            class="btn btn-secondary btn-sm edit-add-modal-button js-ajax-ux-request reset-target-modal-form"
                            data-toggle="modal" data-target="#commonModal" data-progress-bar="hidden"
                    ...
                    <!--add signature - guest users-->
                    @if(config('visibility.doc_client_add_signature') && !auth()->check())
                    <li>
                        <button type="button"
                            class="btn btn-secondary btn-sm edit-add-modal-button js-ajax-ux-request reset-target-modal-form"
                            data-toggle="modal" data-target="#commonModal" data-progress-bar="hidden"
                            data-url="{{ url('contracts/view/'.$document->doc_unique_id.'/sign') }}"
                            data-loading-target="commonModalBody" data-modal-title="@lang('lang.sign_contract')"
                            data-action-form-id="" data-modal-size="modal-lg"
                            data-action-url="{{ url('contracts/view/'.$document->doc_unique_id.'/sign') }}"
                            data-action-method="POST" data-action-ajax-class="js-ajax-ux-request">
                            @lang('lang.sign_contract')
                        </button>
                    </li>
                    ...
                    <!--add signature - client present in person (team member)-->
                    @if(config('visibility.doc_client_add_signature') && auth()->check() && auth()->user()->is_team)
                    <li>
                        <button type="button"
                            class="btn btn-secondary btn-sm edit-add-modal-button js-ajax-ux-request reset-target-modal-form"
                            data-toggle="modal" data-target="#commonModal" data-progress-bar="hidden"
                            data-url="{{ url('contracts/'.$document->doc_unique_id.'/sign/client-in-person') }}"
                            data-loading-target="commonModalBody" data-modal-title="@lang('lang.sign_contract_client_present')"
                            data-action-form-id="" data-modal-size="modal-lg"
                            data-action-url="{{ url('contracts/'.$document->doc_unique_id.'/sign/client-in-person') }}"
                            data-action-method="POST" data-action-ajax-class="js-ajax-ux-request">
                            @lang('lang.client_present_sign')
                        </button>
                    </li>
                    @endif
