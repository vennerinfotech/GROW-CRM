@foreach ($items as $item)
    ...
    <tr id="item_{{ $item->item_id }}" class="{{ $item->pinned_status ?? '' }}">
            ...
            <td class="items_col_checkbox checkitem" id="items_col_checkbox_{{ $item->item_id }}">
                ...
                <span class="list-checkboxes display-inline-block w-px-20">
                    <input type="checkbox" id="listcheckbox-items-{{ $item->item_id }}" name="ids[{{ $item->item_id }}]"
                        ...
                        data-item-id="{{ $item->item_id }}" data-unit="{{ $item->unit->unit_name }}" data-quantity="1"
                        ...
                        data-rate="{{ $item->item_rate }}" data-default-tax="{{ $item->item_default_tax }}"
                        data-item-notes="{{ $item->item_notes }}">
        ...
        <td class="items_col_tableconfig_column_5 {{ config('table.tableconfig_column_5') }} tableconfig_column_5">
            @if (request('itemresource_type') == 'lead')
                <input type="number" class="form-control form-control-sm form-control-number w-px-60"
                    name="quantity[{{ $item->item_id }}]" value="1" min="1">
            @else
                {{ $item->count_sold }}
            @endif
        </td>
                ...
                <span class="list-table-action font-size-inherit">
                    ...
                    @if (config('visibility.action_buttons_edit'))
                        ...
                        <button type="button" title="{{ cleanLang(__('lang.edit')) }}"
                            ...
                            data-action-url="{{ urlResource('/items/' . $item->item_id . '?ref=list') }}"
                    ...
                    <span class="list-table-action">
                        <a href="javascript:void(0);" title="{{ cleanLang(__('lang.pinning')) }}"
                            data-parent="item_{{ $item->item_id }}"
                            data-url="{{ url('/items/' . $item->item_id . '/pinning') }}"
