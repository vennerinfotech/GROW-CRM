    ...
    <tr id="lead_{{ $lead->lead_id }}" class=" {{ $lead->pinned_status ?? '' }}">
        ...
        <td class="col_lead_reminder" id="leads_col_reminder_{{ $lead->lead_id }}">
            @if($lead->reminders->isNotEmpty())
                @php 
                    $reminder = null;
                    //CASE 1: Filters are active - prioritize matching reminder
        if(request()->filled('filter_lead_reminder_date_start') && request()->filled('filter_lead_reminder_date_end')) {
             $reminder = $lead->reminders->whereBetween('reminder_datetime', [
                 \Carbon\Carbon::parse(request('filter_lead_reminder_date_start'))->startOfDay(),
                 \Carbon\Carbon::parse(request('filter_lead_reminder_date_end'))->endOfDay()
             ])->sortByDesc('reminder_datetime')->first();
        } 
        //CASE 2: No filters - Match Lead Logs (Latest Created -> sortByDesc('reminder_id'))
        else {
             //Show the most recently created reminder (Global), appearing at top of logs
             $reminder = $lead->reminders->sortByDesc('reminder_id')->first();
        }
                @endphp
                @if($reminder)
        ...
        <td class="leads_col_action actions_column" id="leads_col_action_{{ $lead->lead_id }}">
            <span class="list-table-action dropdown font-size-inherit">
                ...
                <button type="button" title="@lang('lang.send_email')"
                    ...
                    data-modal-size="modal-xl" data-action-ajax-loading-target="leads-td-container">
            ...
            <span class="list-table-action">
                <button type="button" title="{{ cleanLang(__('lang.star_lead')) }}"
                    class="data-toggle-action-tooltip btn btn-outline-default btn-circle btn-sm opacity-4 ajax-request {{ $lead->is_starred ? 'hidden' : '' }} starred-star-button-{{ $lead->lead_id }}"
            ...
            <!--unstar button-->
            <span class="list-table-action">
                <button type="button" title="{{ cleanLang(__('lang.unstar_lead')) }}"
                    class="data-toggle-action-tooltip btn btn-outline-default btn-circle btn-sm ajax-request text-warning {{ !$lead->is_starred ? 'hidden' : '' }} starred-unstar-button-{{ $lead->lead_id }}"
            ...
            <span class="list-table-action">
                <a href="javascript:void(0);" title="{{ cleanLang(__('lang.pinning')) }}"
                    data-parent="lead_{{ $lead->lead_id }}"
