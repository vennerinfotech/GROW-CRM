<div class="row">
    <div class="col-lg-12">
        ...
        <div id="authorized_date_container" style="display:none;">
            <div class="form-group row">
                <label class="col-sm-12 col-lg-3 text-left control-label col-form-label required">Authorized
                    Date</label>
                <div class="col-sm-12 col-lg-9">
        ...
        <div id="payment_fields_container">
            <div class="form-group row">
                <label class="col-sm-12 col-lg-3 text-left control-label col-form-label required">Mode of
                    Payment</label>
                <div class="col-sm-12 col-lg-9">
        ...
        <div id="completion_details_container" style="display:none;">
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-12 col-lg-9">
                    <input type="hidden" name="refund_image" id="refund_image_hidden"
                        value="{{ $refund->refund_image ?? '' }}">
                    ...
                    <div id="image_preview_container" class="m-t-10">
                        @if (isset($refund) && $refund->refund_image)
                            <div class="m-t-5">
                                <img src="{{ url('storage/files/' . $refund->refund_image) }}" alt="Refund Image"
                                    class="img-thumbnail" style="max-height: 100px;">
                                <div class="m-t-5">
                                    <a href="{{ url('storage/files/' . $refund->refund_image) }}" target="_blank">View
                                        Full Image</a>
                                </div>
                            </div>
                        @endif
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-12 col-lg-9">
                    <textarea class="form-control form-control-sm tinymce-textarea" name="refund_authorized_description" id="refund_authorized_description">{{ $refund->refund_authorized_description ?? '' }}</textarea>
                </div>
            </div>
        </div>

        <!-- Rejected Fields -->
        <div id="rejected_fields"
            style="{{ isset($refund) && $refund->refund_statusid == 5 ? '' : 'display:none;' }}">
            <div class="form-group row">
                <label class="col-sm-12 col-lg-3 text-left control-label col-form-label required">Rejected
                    Reason</label>
                <div class="col-sm-12 col-lg-9">
        ...
        <script>
            $(document).ready(function() {
                // Initialize visibility and logic based on current selection
                ...
                $('#refund_statusid').on('change', function() {
                    ...
                    handleRefundStatusLogic(statusId);
                });

                function handleRefundStatusLogic(statusId) {
                    // Reset readonly by default (enable editing)
                    $('[name="refund_authorized_date"]').prop('readonly', false).css('pointer-events', 'auto').css('background-color', '#fff');
                    $('[name="refund_payment_date"]').prop('readonly', false).css('pointer-events', 'auto').css('background-color', '#fff');

                    // Authorized Status (2)
                    if (statusId == 2) {
                        var $authDate = $('[name="refund_authorized_date"]');
                        if ($authDate.val() == '') {
                             setDateAndHidden('refund_authorized_date', new Date());
                        }
                        // Disable interaction
                        $authDate.prop('readonly', true).css('pointer-events', 'none').css('background-color', '#e9ecef');
                    }
                    ...
                    if (statusId == 3) {
                        ...
                        // Payment Date
                        var $payDate = $('[name="refund_payment_date"]');
                        if ($payDate.val() == '') {
                             setDateAndHidden('refund_payment_date', new Date());
                        }
                        $payDate.prop('readonly', true).css('pointer-events', 'none').css('background-color', '#e9ecef');
                        
                        // Close select2 dropdown if this was a user interaction
                        if (event && event.type === 'change') {
                            $('#refund_statusid').select2('close'); 
                        }
                    }

                    // Rejected Status (5)
                    if (statusId == 5) {
                        // Make dates read-only as per request ("changes no thai")
                        $('[name="refund_authorized_date"]').prop('readonly', true).css('pointer-events', 'none').css('background-color', '#e9ecef');
                        $('[name="refund_payment_date"]').prop('readonly', true).css('pointer-events', 'none').css('background-color', '#e9ecef');
                    }
                }

                // Helper to set date and update hidden field
                function setDateAndHidden(elementName, date) {
                    var $el = $('[name="' + elementName + '"]');
                    $el.datepicker('setDate', date);
                    
                    // Manually format to YYYY-MM-DD for hidden input
                    // Assuming date is a JS Date object
                    var day = ("0" + date.getDate()).slice(-2);
                    var month = ("0" + (date.getMonth() + 1)).slice(-2);
                    var year = date.getFullYear();
                    var mysqlDate = year + "-" + month + "-" + day;
                    
                    $("#" + elementName).val(mysqlDate);
                }

                // Listen for manual changes on datepickers to update hidden field
                $('.pickadate').datepicker().on('changeDate', function(e) {
                    var date = e.date;
                    if(date) {
                         var day = ("0" + date.getDate()).slice(-2);
                        var month = ("0" + (date.getMonth() + 1)).slice(-2);
                ...
                $('#refund_image_input').on('change', function() {
                    ...
                    $('#upload_status').show().text('Uploading...').removeClass('text-danger text-success')
                        .addClass('text-info');
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            if (response.status == 'success') {
                                ...
                                $('#upload_status').text('Upload Successful').removeClass(
                                    'text-info').addClass('text-success');
                                // Update preview
                                var html = '<div class="m-t-5"><img src="' + response.url +
                                    '" class="img-thumbnail" style="max-height: 100px;">' +
                                    '<div class="m-t-5"><a href="' + response.url +
                                    '" target="_blank">View Full Image</a></div></div>';
                                $('#image_preview_container').html(html);
                            } else {
                                $('#upload_status').text('Upload Failed').removeClass('text-info')
                                    .addClass('text-danger');
                            }
                        },
                        error: function() {
                            $('#upload_status').text('Upload Error').removeClass('text-info')
                                .addClass('text-danger');
                        }
                    });
                });
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-12 col-lg-9">
                <select class="select2-basic form-control form-control-sm" id="refund_reasonid"
                    name="refund_reasonid">
                    <option value=""></option>
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-12 col-lg-9">
                <select class="select2-basic form-control form-control-sm" id="refund_courierid"
                    name="refund_courierid">
                    <option value=""></option>
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-12 col-lg-9">
                <input type="text" class="form-control form-control-sm" id="refund_docket_no"
                    name="refund_docket_no" value="{{ $refund->refund_docket_no ?? '' }}">
            </div>
        </div>
