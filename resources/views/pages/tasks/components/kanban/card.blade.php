...
<div class="kanban-card kanban-board-element show-modal-button reset-card-modal-form js-ajax-ux-request {{ $task->pinned_status ?? '' }}"
    ...
    <div class="kanban-card-content-comntainer">
        ...
        <div class="x-meta">
            ...
            <!--pending checklist-->
            @if(config('system.settings_tasks_kanban_pending_checklists') == 'yes')
            <span>
                <strong>{{ cleanLang(__('lang.pending_checklist')) }}:</strong>
                <label class="label {{ $task->pending_checklists_count > 0 ? 'label-warning' : 'label-default' }} p-l-5 p-r-5 p-t-1 p-b-0 m-b-0 vertical-align-middl">
                    {{ $task->pending_checklists_count }}
                </label>
            </span>
            @endif
        ...
        <div class="x-footer row">
            <div class="col-6 x-icons">
                ...
                <span title="{{ cleanLang(__('lang.star_task')) }}"
                    class="x-icon vm p-t-2 data-toggle-action-tooltip opacity-4 font-14 ajax-request {{ $task->is_starred ? 'hidden' : '' }} starred-star-button-{{ $task->task_id }}"
                ...
                <span title="{{ cleanLang(__('lang.unstar_task')) }}"
                    class="x-icon vm p-t-2 data-toggle-action-tooltip font-14 ajax-request text-warning {{ !$task->is_starred ? 'hidden' : '' }} starred-unstar-button-{{ $task->task_id }}"
